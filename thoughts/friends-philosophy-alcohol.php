<?php
/**
 * Страница аудиозаписи: Друзья, философия, коньяк - не помогло. Что теперь?
 */
...
$relatedAudio = getRelatedAudio($audioId, $category, 3);
...
<main class="container mx-auto px-4 py-12">
    <!-- Breadcrumbs -->
    <?= renderBreadcrumbs([
        ['label' => $categoryInfo['title'], 'href' => '/thoughts.php'],
        ['label' => $audio['title']]
    ]) ?>
    ...
    <div class="grid lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
                ...
                <!-- Tags -->
                <?php if (!empty($audio['tags'])): ?>
                    <div class="px-6 pb-6 border-t border-slate-100 pt-6">
                        <h2 class="text-xl font-bold text-slate-900 mb-3">Теги</h2>
                        <div class="flex flex-wrap gap-2">
                            <?php foreach ($audio['tags'] as $tag): ?>
                                <span class="px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm">
                                    #<?= e($tag) ?>
                                </span>
                            <?php endforeach; ?>
                        </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
                    ...
                    <div class="space-y-4">
                        <?php foreach ($relatedAudio as $related): ?>
                            <a href="/<?= e($related['category']) ?>/<?= e(basename($related['id'])) ?>.php" class="block group">
